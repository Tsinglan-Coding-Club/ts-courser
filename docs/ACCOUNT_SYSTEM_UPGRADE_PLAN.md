...
调研日期：2026-09-03。状态：2026-09-07 已实施第一版核心流程，包括关闭公开注册、单租户 Microsoft 登录、Guest 拒绝、教师首次登录待审核、单个本地学生发号、首次凭据设置及受控管理员初始化。2026-09-08 已加入按姓名表格批量创建：系统按姓名生成用户名并处理重名，管理员指定整批统一初始密码；单个创建仍允许管理员指定用户名。学生首次登录只需修改密码，保留发放的用户名。文件导入、批次历史、Entra 停用同步和自动身份关联仍未实施。Microsoft 接口事实与来源见 [Microsoft Entra ID 调研](MICROSOFT_IDENTITY_RESEARCH.md)，当前学生创建流程详见 [管理员界面规格](ACCOUNT_ADMIN_UI_SPEC.md)。

当前学生创建页允许简单初始密码及批量统一密码，保留密码哈希存储、初始密码有效期、首次改密及新密码校验。下文保留原调研和后续方案，其中随机学生登录名、随机初始密码、禁止管理员输入密码等旧规则已被本次课堂使用规则取代；本地管理员的首次凭据设置规则不变。
